<?php 
    ...
    if (isset($_REQUEST['delete_id'])) {
        ...
        $select_stmt = $conn->prepare('SELECT * FROM post WHERE post_id = :id');
        $select_stmt->bindParam(':id', $id);
        $select_stmt->execute();
        $row = $select_stmt->fetch(PDO::FETCH_ASSOC);
        unlink("uploads/posts".$row['img_title']);

        $delete_stmt = $conn->prepare('DELETE FROM post WHERE post_id = :id');
...
<div class="container text-center">
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    <td>ชื่อโพสต์</td>
                    <td>ดูโพสต์ต้นฉบับ</td>
                    <td>ลบ</td>
                </tr>
            </thead>
            <tbody>
                <?php
                    $select_stmt = $conn->prepare('SELECT * FROM post');
                    $select_stmt->execute();
                ...
                ?>
                    <tr>
                        ...
                        <td><a href="/niceclean/post_detail.php?post_id=<?php echo $row['post_id']; ?>" class="btn btn-info">ต้นฉบับ</a></td>
                        <td><a href="?delete_id=<?php echo $row['post_id']; ?>" class="btn btn-danger">ลบ</a></td>
                    </tr>
                <?php } ?>
